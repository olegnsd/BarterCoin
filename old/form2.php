...
if(mail('leadsholding@yandex.ru', "=?utf-8?B?".base64_encode('Заявка на приём BarterCoin '.$_SERVER['SERVER_NAME'])."?=", "Имя: ".htmlspecialchars($_POST[name])."
...
$command_add =  array(
    ...
    "data" => array("line" => array(
        "product" => "чат-бот",
        "ref" => "С лендинга (Заявка на приём BC)",
        "phone" =>$get_phone,
        "name" => $get_name,
        "email" =>$get_email,
        'user_id' => $get_user_id)
