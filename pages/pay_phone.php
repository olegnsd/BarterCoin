...
require('./inc/auth_front.php');
    ...
    $myecho = json_encode(date('d-m-Y H:i:s', time()));
	...
	$myecho = json_encode($_GET['del']);
	`echo " _GET[del]: "  $myecho >>/home/bartercoin/tmp/qaz`;
	$myecho = json_encode($_GET['page']);
	`echo " _GET[page]: "  $myecho >>/home/bartercoin/tmp/qaz`;

if(!$card){include('loginform.php');}else{
if($auth_front){
    if($_POST['add']){ 
        $add_payment = add_payment($account, $card, 'phone_for_pay');
        if($add_payment == "false_phone"){
            $add_payment = '<div class="alert alert-danger" role="alert">Ошибка добавления карты: Неверный телефон</div>';
        ...
        }elseif($add_payment == "false_exists_phone"){
            $add_payment = '<div class="alert alert-danger" role="alert">Ошибка добавления телефона: На этот номер уже зарегистрирована карта</div>';
        }elseif($add_payment == "true_card"){
            $add_payment = '';
        }
    }elseif($_POST['update']){
        $upd_payment = upd_payment($account, $card, 'phone_for_pay');
    }elseif($_GET['del']){
        $del_payment = del_payment($account, $card, 'phone_for_pay');
    }elseif($_GET['sort9']){
...
for($i = 1; $i <= 5; $i++){
    ...
    $temp = "sort_up_dwn_".$i;
    ${$temp} = ' <a href="pay_phone?sort9='.$i.'"><i class="fa fa-chevron-'. $class .'"></i></a>';
...
$res = json_decode(pagination_payment($mysqli, $query1, $num, $card, $account, 'phone_for_pay'), true);
$cards_pay = $res[0];
$navigation = $res[1];
?>
<body >
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
			...
			<form class="form-horizontal" id="form_add" method="POST" action="/pay_phone">
				...
				<div class="form-group">
					<label for="sms" class="col-md-4 control-label">Отправить СМС</label>
					<div class="col-md-6">
						<input id="sms" type="checkbox" class="form-control" name="sms" checked>
						<span class="help-block">
							<strong></strong>
						</span>
					</div>
...
<div class="modal" id="myModal_upd" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Редактирование телефона</h4>
      </div>
      <div class="modal-body">
        <form class="form-horizontal" id="form_upd" method="POST" action="/pay_phone">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a href="/pay_phone?del=" type="button" id='del_butt' key1="" name="send_butt" class="btn btn-danger" >Подтвердить</a>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="panel panel-default">
      <!-- Default panel contents -->
		<caption class="bg-info">
			<h2 class="text-center">Телефоны (для оплаты услуг)</h2>
		</caption>
      <!-- Table -->
      ...
      <table class="table table-hover table-condensed" id='transaction'>
        ...
        foreach($cards_pay as $key1 => $card_pay){?>
          <tr id="add_ch<?=$key1?>" key1="<?=$key1?>">
			<td><?=$card_pay['name']?></td>
			<td id='phone'><?=$card_pay['number']?></td>
			<td><?=$card_pay['amount']?></td>
            <td><?=$card_pay['status']?></td>
            <td><?=$card_pay['date_act']?></td>
			<td>
                <? if($card_pay['status_id'] != 1){?>
                    <a href="javascript:;" action='2' name="update" card_n="<?=$card_pay['id']?>" upd_key1="<?=$key1?>"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
                <?}?>
            </td>
        ...
        <tr>
            <td colspan = "5">
                <div class="input-group">
                    <button class="btn btn-success btn-xs"  data-toggle="modal" data-target="#myModal_add" name="butt_issuse" add_j="<?=$key1+1?>" add_key1="<?=!$key1?-1:$key1?>">Добавить телефон</button>
                </div>
            </td>
        </tr>
        <tr>
...
<script type="text/javascript">
    jQuery(function($) {
        $.mask.definitions['~']='[+-]';
        $('#form_add [name=number]').mask('9(999)-999-99-99',{completed:function(){$('#form_add [name=month]').focus();}});
...
<?}else{?>
	...
	<div class="row"><div class="col-md-4"><div class="form-group"> <input type="number" name="pin" class="form-control" placeholder="КОД"> </div></div>
	    <div class="col-md-8" id="btn1">Забыли код? <a class="btn btn-info submit-button" onclick="if($(this).hasClass('disabled'))return false;$('#btn1 a').addClass('disabled');$('.ajax1').load('<?=$baseHref?>ajax/ajaxreg.php?btn=1&card=<?=$card[id]?>&phone='+encodeURI($('#phone1').val()),function(){$('#btn1 a').removeClass('disabled');});return false;">Выслать новый</a></div>
	</div>
