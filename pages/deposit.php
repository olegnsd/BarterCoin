<?if($_POST['fromnum']){
$card0=getcard($_POST['fromnum']);
if($card0['number']){
$merchant_id = '76387';
$secret_word = 'tcn1gspt';
$order_id = $card0['number'];
...
<?if($_POST['fromnum']){?><div class="alert alert-danger">Вы ввели неверные данные карты</div><?}?>
...
$.mask.definitions['~']='[+-]';
//
$('[name=fromnum]').mask('9999 9999 9999 9999',{completed:function(){$('[name=sum]').focus();}});
...
<p style="color:red;padding: 30px;display:none;">Пополнить можно <br>только карты BarterCoin</p>
</div>
</div></div>
<script>
setTimeout(function(){
...
<div class="col-md-2 hidden-sm hidden-xs"><i style="font-size: 30px;padding-top: 100px;text-align:center;display:block;" class="fa fa-chevron-left"></i></div>
<div class="col-md-2 col-xs-12 hidden-md hidden-lg"><i style="font-size: 30px;text-align:center;display:block;" class="fa fa-chevron-up"></i></div>
<div class="col-md-5 col-xs-12 text-center"><div class="hidden-sm hidden-xs" style="height:100px;"><br><br><br><br></div>
<div class="row"><div class="col-sm-10 col-xs-8"><input type="number" name="sum" class="form-control" value="1000"></div><div class="col-sm-2 col-xs-4" style="      text-align: left;  line-height: 34px;">БР</div></div><br>
<button type="submit" id="submit123" class="btn btn-success btn-lg">Пополнить баланс</button>
