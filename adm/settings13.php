...
if($auth){
	if($_POST['sum']){
		update_min_ball('balance13');
	}

	$balance = mysqli_query($mysqli, "SELECT * FROM balance13 ORDER BY sum ASC");
    
    //проверка счета 9-го банкомата
    $out_count_9 = check_9($mysqli);
    
    $alert_9='';
    if(isset($_POST["time_add"])){//сохранить изменения пополнения 8-го с 9-го
        $time_add = htmlspecialchars($_POST["time_add"]);
        $time_add_8 = json_encode(array("time_add" => $time_add));
        if($_POST['add_7']){
            $add_8 = 1;
        }else{
            $add_8 = 0;
        ...
        $myecho = json_encode($amount." ".$time_add_8." ".$add_8);
        `echo "amount time_add add_8: "  $myecho >>/var/www/tmp/qaz_add_br`;
        
        $info=mysqli_query($mysqli,"UPDATE settings SET delta_time='$add_8', token='$time_add_8' WHERE title='add_9_8'");
        $alert_9 = "<div class='alert alert-success'>Изменения сохранены</div>";
    }else{
        $info=mysqli_query($mysqli,"SELECT * FROM settings WHERE title='add_9_8'");
        $info=mysqli_fetch_assoc($info);
        $time_add = json_decode($info["token"], true);
        $time_add = $time_add["time_add"];
        $add_8 = $info["delta_time"];
    }
...
<!--форма пополнения 8-го с 9-го    -->
<div class="container">
<div class="row">
	<div class="col-md-8 col-md-offset-2">
		<div class="panel panel-default">
			<div class="panel-heading">
				Настройки попоплнения 8-го банкомата с 9-го. <b><i>На счету 9-го банкомата: <?=$out_count_9?>  руб.</i></b>
			</div>

			<div class="panel-body">
				<div id="result"><?=$alert_9?></div>
			</div>
			<form class="form-horizontal" method="POST" action="/adm/?settings=13">
...
<table class="table table-bordered table-fixed">
	<caption class="bg-info">
		<h3 class="text-center">Настройка минимальных балансов из 9-го в 8-й</h3>
	</caption>
	<tr>
		<div class="row">
			<th>Сумма на счету 9-го донора<br>(BCR) меньше(&lt;)</th>
			<th>Пополнять, руб</th>
			<th>Максимум в 8-м банкомате, руб</th>
		</div>
	</tr>
