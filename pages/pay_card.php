...
if($auth_front){
    if($_POST['add']){ 
        $add_payment = add_payment($account, $card, 'card_for_pay');      
        ...
        $upd_payment = upd_payment($account, $card, 'card_for_pay');
    }elseif($_GET['del']){
        $del_payment = del_payment($account, $card, 'card_for_pay');
    }elseif($_GET['sort9']){
...
for($i = 1; $i <= 5; $i++){
    ...
    $temp = "sort_up_dwn_".$i;
    ${$temp} = ' <a href="pay_cadr?sort9='.$i.'"><i class="fa fa-chevron-'. $class .'"></i></a>';
...
$res = json_decode(pagination_payment($mysqli, $query1, $num, $card, $account, 'card_for_pay'), true);
$cards_pay = $res[0];
$navigation = $res[1];   
?>
<body >
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
			...
			<form class="form-horizontal" id="form_add" method="POST" action="/pay_card">
...
<div class="modal" id="myModal_upd" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form-horizontal" id="form_upd" method="POST" action="/pay_card">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a href="/pay_card?del=" type="button" id='del_butt' key1="" name="send_butt" class="btn btn-danger" >Подтвердить</a>
      </div>
    </div>
  </div>
</div>
...
//        alert('key1:'+key1+' mark:'+mark+' card_n:'+card_n);
		...
		$('#del_butt').attr("href", "/pay_card?del="+card_n);
